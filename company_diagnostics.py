...
def start(app,uid,body):
    ...
    with app.db() as c:
        ...
        if c.execute("SELECT 1 FROM company_probes WHERE owner_id=? AND state='running'",(uid,)).fetchone():raise ValueError('Ein Test läuft bereits. Bitte zuerst stoppen.')
...
def worker(uid,provider,ident):
    ...
    app=runtime.configure();provider_budget.initialize(app.DATA_DIR);provider_budget.install_transport();provider_budget.probe_remaining=5
