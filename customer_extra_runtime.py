...
def payload(c,uid,cid):
    if not c.execute('SELECT 1 FROM customers WHERE id=?',(cid,)).fetchone():raise ValueError('Unbekannter Kunde.')
    billing=c.execute('SELECT name,street,house_number,zip_code,city,country FROM customer_billing_addresses WHERE customer_id=?',(cid,)).fetchone()
    locations=[dict(r) for r in c.execute('SELECT id,name,street,house_number,zip_code,city,country,phone FROM customer_locations WHERE customer_id=? ORDER BY name,id',(cid,))]
        ...
        try:
            ...
            with app.db() as c:
                ...
                if path=='/api/v1/customers/billing/save':
                    ...
                    c.execute('DELETE FROM customer_billing_addresses WHERE customer_id=?',(cid,));c.execute('INSERT INTO customer_billing_addresses(owner_id,customer_id,name,street,house_number,zip_code,city,country) VALUES(?,?,?,?,?,?,?,?)',(uid,cid,*vals));after=payload(c,uid,cid)['billing']
                ...
                if path=='/api/v1/customers/location/save':
                    ...
                    if rid:
                        before=c.execute('SELECT * FROM customer_locations WHERE id=? AND customer_id=?',(rid,cid)).fetchone()
                    ...
                    system_features.audit(c,uid,uid,'customer',cid,action,{'location_id':rid,'old':dict(before) if before else {},'new':vals});return self.send_json(200,{'ok':True,'location_id':rid})
                rid=int(body.get('location_id') or 0);row=c.execute('SELECT * FROM customer_locations WHERE id=? AND customer_id=?',(rid,cid)).fetchone()
