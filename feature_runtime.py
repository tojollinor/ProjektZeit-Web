...
def install(app):
    ...
    cd=__import__('customer_data')
    pa=__import__('provider_archive')

    # One admin-managed STARFACE OAuth client for all users. OAuth tokens stay user-bound.
    ...
    def do_GET(self):
        ...
        if path == '/api/v1/integrations':
            ...
            uid=session['id']
            with app.db() as c:
                data=app.integrations.list_configs(c,uid)
                sf=system_features.starface_public(c)
                for item in data:
                    if item['provider']=='starface':
                        item['domain']=sf['domain'];item['username']=sf['client_id'];item['client_configured']=sf['has_client_secret']
                        item['has_secret']=bool(c.execute('SELECT 1 FROM oauth_tokens WHERE owner_id=?',(uid,)).fetchone());item['auth_mode']='oauth'
                try:callback=app.starface_oauth.callback_url()
                except ValueError:callback=''
            return self.send_json(200,{'integrations':data,'starface_callback':callback})
        return previous_get(self)
    app.App.do_GET = do_GET

    # Durable audit trail for time tracking.
    original_edit = app.App.edit_entry
    def edit_entry(self, session, body):
        uid=session['id'];entry_id=int(body.get('id') or 0)
        with app.db() as c:
            row=c.execute('SELECT project_id,category_id,started_at,ended_at,note FROM entries WHERE id=? AND owner_id=?',(entry_id,uid)).fetchone();before=dict(row) if row else {}
        result=original_edit(self,session,body)
        with app.db() as c:
            row=c.execute('SELECT project_id,category_id,started_at,ended_at,note FROM entries WHERE id=? AND owner_id=?',(entry_id,uid)).fetchone();after=dict(row) if row else {}
    ...
    def start_timer(self,session,body):
        ...
        with app.db() as c:
            for row in c.execute('SELECT id,project_id,category_id,started_at,note FROM entries WHERE owner_id=? AND id>? ORDER BY id',(uid,before)):
                system_features.audit(c,uid,uid,'time_entry',row['id'],'created',dict(row),source='manual')
        return result
    ...
    def stop_timer(self,session,body):
        uid=session['id']
        with app.db() as c:rows=[dict(r) for r in c.execute('SELECT id,ended_at FROM entries WHERE owner_id=? AND ended_at IS NULL',(uid,))]
        result=original_stop_timer(self,session,body)
        with app.db() as c:
            for old in rows:
                row=c.execute('SELECT ended_at FROM entries WHERE id=? AND owner_id=?',(old['id'],uid)).fetchone()
    ...
    wrap_create('add_customer','customer','customers');wrap_create('add_project','project','projects');wrap_create('add_category','category','categories')

    # CRM mutation hooks used by customer_runtime.
    ...
    feature_paths={
      '/api/v1/account/context','/api/v1/account/save','/api/v1/account/theme','/api/v1/audit/history','/api/v1/provider/health',
      '/api/v1/admin/starface/config','/api/v1/admin/starface/save','/api/v1/starface/connect/start','/api/v1/archive/start','/api/v1/archive/job',
      '/api/v1/integrations/save','/api/v1/integrations/test','/api/v1/integrations/remove','/api/v1/integrations/list','/api/v1/integrations/ticket'
    }
    def do_POST(self):
        path=urlparse(self.path).path
        ...
        uid=session['id']
        try:
            if path=='/api/v1/integrations/save':return app.App.save_integration(self,session,body)
            if path=='/api/v1/integrations/test':return app.App.test_integration(self,session,body)
            if path=='/api/v1/integrations/remove':return app.App.remove_integration(self,session,body)
            if path in ('/api/v1/integrations/list','/api/v1/integrations/ticket'):return app.App.integration_list(self,session,body)
            ...
            if path=='/api/v1/admin/starface/config':
                with app.db() as c:ac.require_permission(c,uid,'integrations.view');return self.send_json(200,system_features.starface_public(c))
            ...
            if path=='/api/v1/starface/connect/start':
                with app.db() as c:cfg=system_features.starface_public(c)
                if not cfg['has_client_secret']:return self.send_json(409,{'error':'Noch kein Client-Secret hinterlegt. Bitte an den Administrator wenden.'})
            ...
            if path=='/api/v1/archive/job':return self.send_json(200,{'job':system_features.history_job(uid,body.get('provider'))})
...
def serve(app, customer_runtime):
    customer_runtime.install(app);install(app);app.init_db()
