...
class StaffTimeTest(unittest.TestCase):
    def setUp(self):
        ...
        with app.db() as c:
            admin_controls.migrate(c);system_features.migrate(c);workday.migrate(c);st.migrate(c);duty_plan.migrate(c);st.register()
    ...
    def test_missing_stamp_needs_clarification(self):
        self.assertEqual(self.day('2026-09-07')['state'],'unresolved')
