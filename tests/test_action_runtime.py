...
class ActionReceiptTest(unittest.TestCase):
        ...
        class Handler:
            ...
            def do_POST(self):
                ...
                with db() as c:
                    ...
                    if getattr(self,'fail_commit',False):raise RuntimeError('commit failed')
    ...
    def test_rollback_never_reports_success_or_reexecutes_uncertain_action(self):
        self.handler.fail_commit=True;self.handler.do_POST()
        self.assertEqual(self.responses[-1][0],500)
        self.assertEqual(self.responses[-1][1]['action_state'],'uncertain')
        self.assertEqual(self.c.execute('SELECT COUNT(*) FROM changes').fetchone()[0],0)
        self.handler.do_POST();self.assertEqual(self.calls,1)
        self.assertEqual(self.responses[-1][0],409)
...
class MariaActionReceiptTest(unittest.TestCase):
    def setUp(self):
        import test_workday
        test_workday.WorkdayTest.setUp(self)
    def tearDown(self):
        import test_workday
        test_workday.WorkdayTest.tearDown(self)
    def test_receipt_survives_new_database_connection(self):
        import app
        with app.db() as c:actions.migrate(c)
        with app.db() as c:self.assertIsNone(actions.reserve(c,self.uid,'maria-request-00001','/api/v1/customers',{'name':'Kunde'}))
        with app.db() as c:
            row=actions.reserve(c,self.uid,'maria-request-00001','/api/v1/customers',{'name':'Kunde'})
            self.assertEqual(row['state'],'pending')
            c.execute("UPDATE action_receipts SET state='completed',status=200,payload=? WHERE owner_id=? AND request_id=?",(json.dumps({'ok':True}),self.uid,'maria-request-00001'))
        with app.db() as c:
            row=actions.reserve(c,self.uid,'maria-request-00001','/api/v1/customers',{'name':'Kunde'});self.assertEqual(json.loads(row['payload']),{'ok':True})
