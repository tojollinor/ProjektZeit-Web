...
def _safe_debug_text(raw, secret):
    ...
    return ' '.join(text.split())[:1200] or '(nur Leerraum)'


def _response_kind(raw):
    head = raw.lstrip()[:200].lower()
    if head.startswith(b'<?xml') or b'<methodresponse' in head: return 'XML/XML-RPC'
    if head.startswith(b'<!doctype html') or b'<html' in head: return 'HTML'
    if head.startswith((b'{', b'[')): return 'JSON/Text'
    return 'leer' if not raw else 'unbekannt'


def _debug_summary(response, raw, secret):
    content_type = response.getheader('Content-Type') or '(nicht gesetzt)'
    location = response.getheader('Location') or ''
    if secret: location = location.replace(secret, '[TOKEN AUSGEBLENDET]')
    location = re.sub(r'(?i)([?&](?:code|token|access_token|refresh_token|id_token|de\.vertico\.starface\.jwt)=)[^&\s]+', r'\1[AUSGEBLENDET]', location)
    bits = ['HTTP %s' % response.status, 'Content-Type: %s' % content_type[:160], 'Antworttyp: %s' % _response_kind(raw), 'Größe: %s Byte' % len(raw)]
    if location: bits.append('Location: %s' % location[:300])
...
def _plain(value):
    ...
    if isinstance(value, dict):
        return {str(k)[:120]: _plain(v) for k,v in value.items()}
    if isinstance(value, list):
        return [_plain(v) for v in value[:100]]
...
def _external_key(item):
    for key in ('id','callId','uuid','callUuid'):
        ...
        if value not in (None,''):
            return 'starface:%s:%s' % (key,str(value)[:180])
    wire=json.dumps(_plain(item),sort_keys=True,ensure_ascii=False,separators=(',',':'))
    return 'starface:sha256:'+hashlib.sha256(wire.encode()).hexdigest()


def _customer_hint(description,item):
    ...
    phones=[]
    for key in ('callerNumber','calledNumber'):
        value=item.get(key)
        ...
        try:
            ...
            for key, value in response.getheaders():
                if key.lower() == 'set-cookie':
                    jar = SimpleCookie(); jar.load(value); self.cookies.update({k: m.value for k,m in jar.items()})
            chunks, size, started = [], 0, time.monotonic()
            while True:
                if time.monotonic()-started > 12: raise ValueError('Zeitlimit beim Lesen der STARFACE-Anrufliste.')
                chunk = response.read1(16384)
                if not chunk: break
                size += len(chunk)
                if size > 1048576: raise ValueError('STARFACE-Anrufliste ist zu groß. Kürzeren Zeitraum wählen.')
                chunks.append(chunk)
            raw = b''.join(chunks); debug = _debug_summary(response, raw, self.secret)
            if response.status != 200: raise ValueError('STARFACE-Anrufliste nicht erreichbar. STARFACE-Debug: ' + debug)
            if b'<!DOCTYPE' in raw.upper() or b'<!ENTITY' in raw.upper(): raise ValueError('Ungültige XML-Antwort von STARFACE. STARFACE-Debug: ' + debug)
            try:
                result, _ = loads(raw, use_builtin_types=False)
            except Fault as error:
                code = error.faultCode if type(error.faultCode) is int else 'unbekannt'
                raise ValueError('STARFACE hat die UCI-Abfrage abgewiesen (Code %s). STARFACE-Debug: %s' % (code, debug)) from None
            ...
            if len(result) != 1: raise ValueError('Unerwartete STARFACE-Antwort. STARFACE-Debug: ' + debug)
            return result[0]
        except OSError:
...
def start_time(value):
    ...
    if isinstance(value, DateTime): value = value.value
    if isinstance(value, str):
        for parser in (lambda v: datetime.fromisoformat(v.replace('Z','+00:00')), lambda v: datetime.strptime(v,'%Y%m%dT%H:%M:%S')):
            try: return start_time(parser(value))
...
def load(config, rpc_factory=UciClient):
    ...
    if type(days) is not int or days not in (7, 30, 90, 365): raise ValueError('Ungültiger Zeitraum für die Anrufliste.')
    if type(offset) is not int or not 0 <= offset <= 100000: raise ValueError('Ungültige Seite der Anrufliste.')
    mode = config.get('call_type', 'all')
    filters = {'all': ('', ''), 'inbound': ('INBOUND', ''), 'outbound': ('OUTBOUND', ''), 'missed': ('INBOUND', 'MISSED')}
    if not isinstance(mode, str) or mode not in filters: raise ValueError('Ungültiger Anruffilter.')
    direction_filter, result_filter = filters[mode]
    now = datetime.now(timezone.utc); rpc = rpc_factory(config)
    if rpc.call('connection.login') is not True: raise ValueError('STARFACE-UCI-Anmeldung abgewiesen. Berechtigungen prüfen.')
    try:
        payload = rpc.call('callList.getCallList', (now.replace(tzinfo=None), (now-timedelta(days=days)).replace(tzinfo=None), direction_filter, result_filter, 'NON_GROUP', 'startTime', 'DESCENDING', offset, 100))
    finally:
        try: rpc.call('connection.logout')
        except (ValueError, OSError): pass
    if not isinstance(payload, dict) or not isinstance(payload.get('entries'), list): raise ValueError('STARFACE lieferte keine gültige Anrufliste.')
    rows, skipped, raw_names = [], 0, set()
    for item in payload['entries'][:100]:
        ...
        if (direction_filter and direction != direction_filter) or (result_filter and result != result_filter): skipped += 1; continue
        start = start_time(item.get('startTime')); seconds = item.get('duration'); valid_duration = type(seconds) is int and 0 <= seconds <= 31536000
        try: end = start + timedelta(seconds=seconds) if start and valid_duration else None
        except OverflowError: end = None
        number = item.get('calledNumber'); number = number if isinstance(number, str) and number else 'Unbekannt'
    ...
    return {'columns': ['Richtung', 'Status', 'Anrufer', 'Angerufene Rufnummer', 'Name / Beschreibung', 'Anrufbeginn', 'Ende (berechnet)', 'Dauer'],
            ...
            'note': f'Persönliche Anrufe (ohne Gruppenanrufe) · letzte {days} Tage · alle von STARFACE gelieferten Felder sind als Rohdaten verfügbar.' +
