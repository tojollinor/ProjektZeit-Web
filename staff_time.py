...
PERMISSIONS={
 ...
 'staff.manage':'Arbeitszeitmodelle und Urlaubskonten verwalten',
...
def migrate(c):
    c.executescript('''
    CREATE TABLE IF NOT EXISTS staff_models(id INTEGER PRIMARY KEY,user_id INTEGER NOT NULL,valid_from VARCHAR(10) NOT NULL,mode VARCHAR(12) NOT NULL,target_seconds INTEGER NOT NULL,weights_json LONGTEXT NOT NULL,subdivision VARCHAR(8) NOT NULL,opening_seconds INTEGER NOT NULL DEFAULT 0,created_by INTEGER NOT NULL,created_at VARCHAR(40) NOT NULL,UNIQUE(user_id,valid_from));
...
def model_list(c,uid):return [dict(r,weights=json.loads(r['weights_json'])) for r in c.execute('SELECT * FROM staff_models WHERE user_id=? ORDER BY valid_from',(uid,))]
...
def holiday_calendar(year,subdivision):return holidays.country_holidays('DE',subdiv=subdivision,years=[year],language='de')
...
def daily(data,day,clock=None):
    ...
    for w in data['work']:
        x=parse(w['started_at']);y=parse(w['ended_at']) if w['ended_at'] else clock
        if x<b and y>a:
            spans.append((max(x,a),min(y,b)));unclosed|=not w['ended_at']
    pauses=[(max(parse(p['started_at']),a),min(parse(p['ended_at']) if p['ended_at'] else clock,b)) for p in data['pauses'] if parse(p['started_at'])<b and (not p['ended_at'] or parse(p['ended_at'])>a)]
    worked=sum(int((y.astimezone(timezone.utc)-x.astimezone(timezone.utc)).total_seconds()) for x,y in subtract(merge(spans),pauses))
    ...
    unresolved=bool(target and day<clock.astimezone(TZ).date() and (unclosed or (not spans and not absent and not confirmed)))
    complete=b<=clock and not unresolved and target is not None
    delta=worked+credit+reduction-(target or 0)
    return {'day':str(day),'target_seconds':target,'worked_seconds':worked,'credit_seconds':credit,'reduction_seconds':reduction,'delta_seconds':delta,'posted_seconds':delta if complete else 0,'state':'closed' if complete else 'unresolved' if unresolved else 'unconfigured' if target is None else 'planned' if a>clock else 'provisional','holiday':holiday,'labels':labels,'vacation_units':vacation}
...
def month_report(c,uid,month):
    ...
    return {'projects':projects,'days':rows,'month':month,'requests':data['requests'],'work':data['work'],'models':data['models'],'movements':movements,'closure':closed,'posted_seconds':closed['posted_seconds'] if closed else sum(x['posted_seconds'] for x in rows),'movement_seconds':sum(x['seconds'] for x in movements if x['day']<=str(now().astimezone(TZ).date())),'revision_seconds':sum(x['posted_seconds'] for x in rows)-(closed['posted_seconds'] if closed else sum(x['posted_seconds'] for x in rows))}

def vacation_balance(c,uid,year,extra_requests=None):
    start=date(year,1,1);end=date(year+1,1,1);data=dataset(c,uid,start,end)
    if extra_requests:data['requests'].extend(extra_requests)
    approved=sum(daily(data,d)['vacation_units'] for d in days(start,end));taken=sum(daily(data,d)['vacation_units'] for d in days(start,min(end,now().astimezone(TZ).date())))
    ...
    expired=bool(account and account['carry_until'] and str(now().astimezone(TZ).date())>account['carry_until'])
    if expired:
        expiry=date.fromisoformat(account['carry_until'])+timedelta(days=1)
        consumed=sum(daily(data,d)['vacation_units'] for d in days(start,min(expiry,end)))
        carry=min(carry,consumed)
    ...
    planned=sum(daily(hypothetical,d)['vacation_units'] for d in days(start,end))
    return {'year':year,'configured':bool(account),'entitlement':entitlement,'carry':carry,'taken':taken,'approved':approved,'pending':max(0,planned-approved),'available':entitlement+carry-approved,'after_pending':entitlement+carry-planned}

def save_model(c,actor,body):
    require(c,actor,'staff.manage');uid=int(body['user_id']);person(c,uid);valid=date.fromisoformat(body['valid_from'])
    if c.execute('SELECT 1 FROM staff_month_closures WHERE user_id=? AND month>=? LIMIT 1',(uid,str(valid)[:7])).fetchone():raise ValueError('Modell würde einen abgeschlossenen Monat verändern.')
    mode=body['mode'];weights=[int(x) for x in body['weights']]
    if mode not in ('daily','weekly','monthly') or len(weights)!=7 or min(weights)<0 or max(weights)>100 or not sum(weights):raise ValueError('Ungültiges Arbeitszeitmodell.')
    subdiv=str(body['subdivision'])
    if subdiv not in holidays.DE.subdivisions:raise ValueError('Bundesland auswählen.')
    target=seconds(body['hours'])
    if target<=0:raise ValueError('Sollstunden müssen positiv sein.')
    if c.execute('SELECT 1 FROM staff_models WHERE user_id=? AND valid_from=?',(uid,str(valid))).fetchone():raise ValueError('Für dieses Datum existiert bereits ein Modell. Bitte ein neues Gültigkeitsdatum wählen.')
    opening=seconds(abs(Fraction(str(body.get('opening_hours',0)))))*(1 if Fraction(str(body.get('opening_hours',0)))>=0 else -1)
    if model_list(c,uid) and opening:raise ValueError('Startsaldo nur beim ersten Modell; weitere Korrekturen über Buchhaltung.')
    c.execute('INSERT INTO staff_models(user_id,valid_from,mode,target_seconds,weights_json,subdivision,opening_seconds,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?)',(uid,str(valid),mode,target,json.dumps(weights),subdiv,opening,actor,iso(now())))
    audit(c,actor,uid,'work_model',uid,'created',body);return {'ok':True}
...
def cancellation(c,actor,body):
    r=c.execute("SELECT * FROM staff_requests WHERE id=? AND user_id=? AND state='approved'",(int(body['id']),actor)).fetchone()
...
def account(c,actor,body):
    ...
    if until:date.fromisoformat(until)
...
def handle(c,uid,action,body):
    ...
    if action=='report':
        ...
        report['vacation']=vacation_balance(c,target,int(month[:4]));report['account']=account_balance(c,target);return report
    ...
    if action=='calendar':
        ...
        return {'events':calendar_data(c,uid,a,b)}
...
def conflicts(c,uid,a,b,kind,exclude=0,approved_only=False):
    ...
    for r in data['requests']:
        ...
        if kind=='sick' and r['payload']['kind'].get('vacation'):continue
...
def account_balance(c,uid,through=None):
    ...
    total=models[0]['opening_seconds'];unresolved=0;cursor=start.replace(day=1)
    while cursor<end:
        next_month=(cursor.replace(day=28)+timedelta(days=4)).replace(day=1);key=str(cursor)[:7]
        if key in closures:total+=closures[key]['posted_seconds']
        else:
            first=max(cursor,start);last=min(next_month,end);data=dataset(c,uid,first,last)
...
def absence_preview(c,uid,body):
    ...
    years=[]
    for year in range(a.astimezone(TZ).year,(b-timedelta(seconds=1)).astimezone(TZ).year+1):
        before=vacation_balance(c,target,year);after=vacation_balance(c,target,year,[proposal]);years.append({'year':year,'days_requested':(before['after_pending']-after['after_pending'])/1_000_000,'remaining_days':after['after_pending']/1_000_000,'configured':before['configured']})
