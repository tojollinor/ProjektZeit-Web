...
def workspace(c,uid,body,allow_team=False):
    ...
    for e in rows:
        ...
        concurrent={x['project_id'] for x in rows if x['source']=='manual' and x['owner_id']==uid and parse(x['start']) and a and b and parse(x['start'])<b and (parse(x['end']) or datetime.now(timezone.utc))>a}
