(()=>{
 ...
 async function rebuildHistory(box,data){const history=$q('[data-pane="history"]',box);if(!history)return;const map=await starfaceUsers(),events=data.activity||[];const list=document.createElement('div');list.className='activity-list';for(const event of events){const r=event.raw||{},article=document.createElement('article');if(event.provider==='starface'){const [kind,title,mark]=callKind(r);article.innerHTML=`<span class="provider-label">STARFACE</span><div class="call-line"><span class="call-status ${kind}" title="${a(title)}" aria-label="${a(title)}"><b>☎</b><i>${e(mark)}</i></span><strong>${e(party(r.callerNumber,map))} → ${e(party(r.calledNumber,map))}</strong><span>${e(formatDuration(r))}</span></div><small>${e(event.occurred_at?new Date(event.occurred_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'}):'Zeitpunkt unbekannt')}</small>`;}else if(event.provider==='teamviewer'){article.innerHTML=`<span class="provider-label">TEAMVIEWER</span><strong>${e(r.devicename||r.device_name||'Gerät')} · ${e(r.username||'–')}</strong><small>${e(r.start_date||event.occurred_at||'')}</small>`;}else{article.innerHTML=`<span class="provider-label">ZAMMAD</span><strong>${e(r.number||r.id||'')} · ${e(r.title||event.summary||'')}</strong><small>${e(r.updated_at||event.occurred_at||'')}</small>`;}list.append(article);}if(!events.length){list.innerHTML='<p class="muted">Noch kein zuordenbarer Verlauf vorhanden.</p>';}history.replaceChildren(list);}
