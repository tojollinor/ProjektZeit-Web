(()=>{
 ...
 logsSection.innerHTML=`<article class="panel logs-panel"><div class="panel-head"><div><p class="eyebrow">BETRIEBSJOURNAL</p><h3>Logs</h3></div><button class="secondary" data-log-refresh>Aktualisieren</button></div><div class="provider-toolbar log-toolbar"><label>Kategorie<select data-log-category><option value="">Alle Kategorien</option><option value="starface">STARFACE</option><option value="teamviewer">TeamViewer</option><option value="zammad">Zammad</option><option value="system">System</option></select></label><label>Status<select data-log-level><option value="">Alle Status</option><option value="info">Info</option><option value="success">Erfolg</option><option value="warning">Warnung</option><option value="error">Fehler</option></select></label><label>Anzeigen<select data-log-limit><option value="25">25</option><option value="50">50</option><option value="100">100</option><option value="150">150</option><option value="all">Alle</option></select></label></div><p class="list-status" data-log-status></p><div class="log-table-wrap"><table class="log-table"><thead><tr><th>Status</th><th>Zeitpunkt</th><th>Bereich</th><th>Aktion</th><th>Meldung</th><th>Details</th></tr></thead><tbody data-log-list></tbody></table></div></article>`;
 $q('main')?.insertBefore(logsSection,$q('#toast'));
 const logDetails=document.createElement('dialog');logDetails.className='log-table-dialog';logDetails.innerHTML='<div class="panel-head"><h3>Log-Details</h3><button type="button" class="secondary" data-close>Schließen</button></div><div data-log-table-detail></div>';$q('body').append(logDetails);$q('[data-close]',logDetails).onclick=()=>logDetails.close();
 async function renderLogs(){
  const category=$q('[data-log-category]',logsSection).value,level=$q('[data-log-level]',logsSection).value,limit=$q('[data-log-limit]',logsSection).value,status=$q('[data-log-status]',logsSection),list=$q('[data-log-list]',logsSection);
  status.textContent='Logs werden geladen …';
  try{const data=await post('/api/v1/logs/list',{category,level,limit});const rows=data.logs||[];list.replaceChildren();for(const row of rows){const tr=document.createElement('tr');tr.className=`log-table-row ${row.level}`;const details=row.details&&Object.keys(row.details).length?JSON.stringify(row.details,null,2):'';tr.innerHTML=`<td><span class="log-level"><i></i>${e(row.level==='success'?'Erfolg':row.level==='warning'?'Warnung':row.level==='error'?'Fehler':'Info')}</span></td><td>${e(new Date(row.created_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'}))}</td><td>${e(labelProvider(row.category))}</td><td>${e(row.action)}</td><td>${e(row.message)}</td><td>${details?'<button type="button" class="secondary subtle icon-action" data-details aria-label="Technische Details anzeigen" title="Details anzeigen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5c5.5 0 9.6 5.1 10 5.7a2.1 2.1 0 0 1 0 2.6C21.6 13.9 17.5 19 12 19S2.4 13.9 2 13.3a2.1 2.1 0 0 1 0-2.6C2.4 10.1 6.5 5 12 5Zm0 2c-4.2 0-7.6 3.7-8.3 5 .7 1.3 4.1 5 8.3 5s7.6-3.7 8.3-5c-.7-1.3-4.1-5-8.3-5Zm0 2.2a2.8 2.8 0 1 1 0 5.6 2.8 2.8 0 0 1 0-5.6Z"/></svg></button>':'–'}</td>`;const button=$q('[data-details]',tr);if(button)button.onclick=()=>{$q('[data-log-table-detail]',logDetails).innerHTML=`<dl><dt>Bereich</dt><dd>${e(labelProvider(row.category))}</dd><dt>Aktion</dt><dd>${e(row.action)}</dd><dt>Meldung</dt><dd>${e(row.message)}</dd></dl><pre>${e(details)}</pre>`;logDetails.showModal();};list.append(tr);}if(!rows.length)list.innerHTML='<tr><td colspan="6" class="empty">Keine passenden Logeinträge.</td></tr>';const total=Number.isFinite(Number(data.total))?Number(data.total):rows.length;status.textContent=limit==='all'?`${total} Einträge`:`${rows.length} von ${total} Einträgen angezeigt`;}catch(error){status.textContent=error.message;pzToast(error.message,'error');}
 }
 logsButton.onclick=()=>{showView('logs');$q('#page-title').textContent='Logs';renderLogs();};
 $q('[data-log-refresh]',logsSection).onclick=renderLogs;$q('[data-log-category]',logsSection).onchange=renderLogs;$q('[data-log-level]',logsSection).onchange=renderLogs;$q('[data-log-limit]',logsSection).onchange=renderLogs;
