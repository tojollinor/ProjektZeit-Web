...
(()=>{
 ...
 function mount(host,scope={}){
  ...
  panel.innerHTML=`<div class="panel-head"><div><p class="eyebrow">ZEITEN PRÜFEN UND ZUORDNEN</p><h3>${scope.customer_id?'Kundenzeitstrahl':scope.projectView?'Übersicht':'Mein Arbeitstag'}</h3></div><button class="secondary" data-tw-refresh aria-label="Zeiten aktualisieren">↻</button></div><div class="tw-toolbar"><label>Datum<input type="date" data-tw-day></label><label>Zeitraum<select data-tw-days><option value="1">Ein Tag</option><option value="7">7 Tage ab Datum</option><option value="30">30 Tage ab Datum</option></select></label><label>Projekt<select data-tw-project><option value="">Alle Projekte</option></select></label><label data-tw-team-label hidden><input type="checkbox" data-tw-team> Team anzeigen</label></div><div class="tw-tabs" role="tablist" aria-label="Zeitansicht"><button class="secondary" data-tw-mode="timeline">${scope.projectView?'Übersicht':'Zeitstrahl'}</button><button class="secondary" data-tw-mode="inbox">Zuordnungseingang</button><button class="secondary" data-tw-mode="review">Tagesabschluss</button><button class="secondary" data-tw-mode="bookings">Geprüfte Buchungen</button></div><p data-tw-status role="status"></p><div data-tw-summary class="tw-summary"></div><div data-tw-content></div>`;
