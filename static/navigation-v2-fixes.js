(()=>{
 ...
 function syncWorkPanel(name=activeName()){const panel=q('.work-panel');if(!panel)return;panel.hidden=name==='statistics'||name.startsWith('settings-')||name.startsWith('workshop-')||['admin-options','staff-settings','absence-approvals','logs','bookkeeping','notifications','account'].includes(name);}
