(()=>{
 ...
 // The previous version observed the complete document forever and rescanned it for every DOM change.
 // A short bounded settle period plus navigation-triggered refreshes is enough for asynchronously added UI.
 let tries=0;const wait=setInterval(()=>{tries++;apply();if(tries>40)clearInterval(wait);},100);
 ...
 document.addEventListener('click',event=>{
  ...
  if(event.target.closest('.sidebar'))setTimeout(scheduleApply,60);
 },true);
 ...
 function script(src,key){if(document.querySelector(`script[data-${key}]`))return;const el=document.createElement('script');el.src=versioned(src);el.async=false;el.defer=true;el.dataset[key]='';document.body.append(el);}
 ...
 stylesheet('/company-ui.css','companyUi');script('/company-ui.js','companyUi');script('/notifications.js','notifications');
})();
