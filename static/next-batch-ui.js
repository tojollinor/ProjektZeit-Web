(()=>{
 ...
 function adminAccordion(){
  ...
  const items=[
   ...
   ['smtp','E-Mail / SMTP',true,'smtp.view'],['notifications','Benachrichtigungen',true,'notifications.edit'],['integrations','Integrationen',true,'integrations.view'],['api','API',true,'admin.options.view'],['super','Systemschutz',true,'system.options.edit']
  ];
 ...
 async function changeProjectStatus(id,status){let send=false;if(status==='closed')send=await window.pzUI.confirm('Projekt schließen und direkt zur Abrechnung übergeben?\n\nOK = schließen und zur Abrechnung\nAbbrechen = nur schließen');try{if(send&&window.pzCompany?.closeForBilling){await window.pzCompany.closeForBilling(id);return;}await post('/api/v1/projects/status',{project_id:id,status,send_to_billing:send});notify(status==='active'?'Projekt aktiviert':status==='parked'?'Projekt geparkt':send?'Projekt geschlossen und zur Abrechnung übergeben':'Projekt geschlossen','success');await refresh();await renderProjects();}catch(error){notify(error.message,'error');}}
 ...
 const widgetLabels={stats:'Kennzahlen',missed_calls:'Verpasste Anrufe',timeline:'Tagesverlauf',entries:'Letzte Stempelungen'};
 function dashboardUi(){
  const dash=q('#view-dashboard');if(!dash||q('[data-dashboard-edit]',dash))return;
  const edit=document.createElement('button');edit.type='button';edit.className='secondary subtle dashboard-edit';edit.dataset.dashboardEdit='';edit.textContent='Dashboard bearbeiten';dash.prepend(edit);
  const stats=q('.stats',dash),timeline=q('.timeline-panel',dash),entries=q('#entry-table',dash)?.closest('.panel');if(stats)stats.dataset.dashboardWidget='stats';if(timeline)timeline.dataset.dashboardWidget='timeline';if(entries)entries.dataset.dashboardWidget='entries';
  const missed=document.createElement('article');missed.className='panel missed-calls-widget';missed.dataset.dashboardWidget='missed_calls';missed.innerHTML='<div class="panel-head"><div><p class="eyebrow">STARFACE</p><h3>Verpasste Anrufe</h3></div><button type="button" class="secondary" data-missed-refresh>Aktualisieren</button></div><p class="muted" data-missed-status>Lokaler Stand</p><div data-missed-list></div>';dash.append(missed);
  const grid=document.createElement('div');grid.className='dashboard-grid';dash.append(grid);for(const el of qa('[data-dashboard-widget]',dash))grid.append(el);
  q('[data-missed-refresh]',missed).onclick=()=>refreshMissed(true);edit.onclick=openWidgetEditor;refreshMissed(false);applyDashboard();
 }
 async function applyDashboard(){const dash=q('#view-dashboard'),grid=q('.dashboard-grid',dash);if(!grid)return;try{if(!context)await loadContext();const layout=context.dashboard||{},widgets=layout.widgets||Object.keys(widgetLabels);for(const [i,key] of widgets.entries()){const el=q(`[data-dashboard-widget="${key}"]`,grid);if(el){el.style.order=i;el.style.setProperty('--widget-height',(layout.heights?.[key]||420)+'px');}}for(const el of qa('[data-dashboard-widget]',grid))el.hidden=!widgets.includes(el.dataset.dashboardWidget);}catch(error){notify(error.message,'error');}}
 function openWidgetEditor(){
  let selected=[...(context?.dashboard?.widgets||Object.keys(widgetLabels))];const heights={...context?.dashboard?.heights},list=q('[data-widget-editor-list]',widgetEditor);
  function paint(){const ordered=[...selected,...Object.keys(widgetLabels).filter(k=>!selected.includes(k))];list.innerHTML=ordered.map(key=>{const pos=selected.indexOf(key);return `<div class="widget-editor-row"><label><input type="checkbox" data-widget-check="${key}" ${pos>=0?'checked':''}>${h(widgetLabels[key])}</label><label>Höhe (px)<input type="number" min="240" max="1200" step="20" data-widget-height="${key}" value="${heights[key]||420}"></label><div><button type="button" class="secondary subtle" data-widget-up="${key}" ${pos<=0?'disabled':''} aria-label="Nach oben">↑</button><button type="button" class="secondary subtle" data-widget-down="${key}" ${pos<0||pos===selected.length-1?'disabled':''} aria-label="Nach unten">↓</button></div></div>`;}).join('');
   qa('[data-widget-height]',list).forEach(x=>x.oninput=()=>heights[x.dataset.widgetHeight]=Math.max(240,Math.min(1200,Number(x.value)||420)));
   qa('[data-widget-check]',list).forEach(x=>x.onchange=()=>{const key=x.dataset.widgetCheck;if(x.checked&&!selected.includes(key))selected.push(key);if(!x.checked)selected=selected.filter(v=>v!==key);paint();});
   qa('[data-widget-up],[data-widget-down]',list).forEach(b=>b.onclick=()=>{const i=selected.indexOf(b.dataset.widgetUp||b.dataset.widgetDown),j=i+(b.dataset.widgetUp?-1:1);if(i>=0&&j>=0&&j<selected.length)[selected[i],selected[j]]=[selected[j],selected[i]];paint();});
  }paint();let saving=false;q('[data-widget-editor-save]',widgetEditor).onclick=async()=>{if(saving)return;saving=true;const button=q('[data-widget-editor-save]',widgetEditor);button.disabled=true;try{const d=await post('/api/v1/dashboard/preferences',{widgets:selected,heights});context.dashboard=d;await applyDashboard();widgetEditor.close();notify('Dashboard gespeichert','success');}catch(error){notify(error.message,'error');}finally{saving=false;button.disabled=false;}};widgetEditor.showModal();
 }

 async function refreshMissed(syncServer){const status=q('[data-missed-status]'),list=q('[data-missed-list]');if(!list)return;try{if(syncServer){status.textContent='STARFACE wird vollständig abgeglichen …';const started=await post('/api/v1/archive/start',{provider:'starface'}),id=started.job?.id;await new Promise((resolve,reject)=>{let tries=0;const poll=async()=>{try{const d=await post('/api/v1/archive/job',{provider:'starface'}),j=d.job||{};if(id&&j.id&&j.id!==id){if(++tries<300)return setTimeout(poll,1000);}if(j.state==='running')return setTimeout(poll,1000);if(j.state==='success')resolve();else reject(new Error(j.error||'STARFACE-Abgleich fehlgeschlagen'));}catch(e){reject(e);}};poll();});}const d=await post('/api/v1/starface/missed',{}),calls=d.calls||[];status.textContent=`${calls.length} nicht zurückgerufene ${calls.length===1?'Anruf':'Anrufe'}`;list.innerHTML=calls.length?calls.map(c=>`<article class="missed-call-row" data-call-key="${a(c.external_key)}"><div><strong>${h(c.name||c.number||'Unbekannt')}</strong><small>${h(c.number||'')} · ${h(fmtDate(c.occurred_at))}</small></div><a class="secondary subtle" href="tel:${a(String(c.number||'').replace(/[^+\d]/g,''))}">Anrufen</a></article>`).join(''):'<p class="muted">Keine offenen Rückrufe.</p>';if(!d.server_write_supported){const note=document.createElement('small');note.className='muted callback-write-note';note.textContent='Der STARFACE-Rückrufstatus wird beim Aktualisieren vollständig vom Server abgeglichen.';list.append(note);}}catch(error){status.textContent=error.message;}}
 ...
 function providerStatus(){qa('.provider-view').forEach(s=>window.pzProviderCount?.(s));}
