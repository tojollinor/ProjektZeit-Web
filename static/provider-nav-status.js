(()=>{
 ...
 let pendingProvider='';
 q('[data-provider-connect-no]',dialog).onclick=()=>{pendingProvider='';dialog.close();};
 function decorate(){
 ...
 function goSettings(provider){
  ...
  setTimeout(()=>{
   const card=q(`#integration-cards [data-provider="${provider}"]`);if(!card)return;
 ...
 document.addEventListener('click',event=>{
  if(event.target.closest('[data-integration-action],[data-pz-starface-connect],[data-refresh]'))setTimeout(load,1600);
 });
