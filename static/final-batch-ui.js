(()=>{
 ...
 const detail=document.createElement('dialog');detail.className='pz-provider-detail';detail.innerHTML='<div class="panel-head pz-provider-detail-head"><div><p class="eyebrow">VERBINDUNGSDETAIL</p><h3 data-pz-detail-title>Details</h3></div><button type="button" class="secondary icon-action" data-pz-detail-close aria-label="Schließen" title="Schließen"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6.4 5 5.6 5.6L17.6 5 19 6.4 13.4 12l5.6 5.6-1.4 1.4-5.6-5.6L6.4 19 5 17.6l5.6-5.6L5 6.4 6.4 5Z"/></svg></button></div><div data-pz-detail-body></div>';document.body.append(detail);q('[data-pz-detail-close]',detail).onclick=()=>detail.close();
 ...
 async function chooseCustomer(provider,row){
  const list=await getCustomers(),body=q('[data-pz-assign-body]',assign),suggestions=row.assignment?.suggestions||[],labels={email:'E-Mail-Adresse',phone:'Rufnummer',teamviewer_id:'TeamViewer-ID',zammad_customer_id:'Zammad-Kunden-ID',zammad_organization_id:'Zammad-Organisations-ID'};body.innerHTML=`<fieldset class="assignment-suggestions"><legend>Vorgeschlagene Zuordnungsmerkmale</legend>${suggestions.length?suggestions.map((item,index)=>`<label><input type="radio" name="match" value="${index}" ${index===0?'checked':''}> <span>${h(labels[item.type]||item.type)}: <strong>${h(item.value)}</strong></span></label>`).join(''):'<p class="muted">In diesem Eintrag wurde kein stabiles Merkmal gefunden. Bei Zammad werden insbesondere Kunden-E-Mail, Kunden-ID und – sofern vorhanden – Organisation vorgeschlagen.</p>'}</fieldset><label>Kunde<select data-pz-customer><option value="">Kunde wählen …</option>${list.map(c=>`<option value="${c.id}">${h(c.name)}</option>`).join('')}</select></label><div class="panel-actions"><button type="button" class="primary" data-pz-assign-one>Nur diesen Eintrag</button><button type="button" class="secondary" data-pz-assign-all ${suggestions.length?'':'disabled'}>Alle passenden zuordnen</button></div><p class="list-status" data-pz-assign-status></p>`;
  async function go(bulk){const cid=Number(q('[data-pz-customer]',body).value),status=q('[data-pz-assign-status]',body),selected=suggestions[Number(q('input[name=match]:checked',body)?.value||0)];if(!cid){status.textContent='Bitte einen Kunden auswählen.';return;}try{status.textContent='Zuordnung wird gespeichert …';const r=await pzPost('/api/v1/provider/assign/customer',{provider,external_key:row.external_key,customer_id:cid,bulk,match_type:selected?.type||'',match_value:selected?.value||''});status.textContent=bulk?`${r.matched||1} passende Einträge zugeordnet.`:'Eintrag zugeordnet.';notify('Kundenzuordnung gespeichert','success');setTimeout(()=>{assign.close();q(`#view-${provider} [data-refresh]`)?.click();},450);}catch(e){status.textContent=e.message;}}
 ...
 async function openProviderDetail(provider,row){
  ...
  body.innerHTML=`<section class="pz-detail-assignment"><span class="pz-assignment-dot ${assignmentState(row)}"></span><div class="pz-assignment-copy"><small>ABRECHNUNGSZUORDNUNG</small><strong>${as.project_name?'Vollständig zugeordnet':as.customer_name?'Projekt fehlt':'Kunde und Projekt fehlen'}</strong></div>${customer}${as.project_name?`<div class="pz-assignment-fact"><small>Projekt</small><strong>${h(as.project_name)}</strong></div>`:''}<div class="panel-actions"><button class="secondary" data-detail-customer>${as.customer_id?'Kunde ändern':'Kunde zuordnen'}</button><button class="primary" data-detail-project>${as.project_id?'Projekt ändern':'Projekt zuordnen'}</button></div></section><section class="pz-provider-facts"><h4>${provider==='teamviewer'?'Verbindungsdaten':'Gesprächsdaten'}</h4><dl class="pz-raw-fields">${providerFacts(provider,row)}</dl></section>`;
 ...
 function decorateProvider(provider){
  ...
  trs.forEach((tr,i)=>{const row=tr._pzRow;if(!row)return;tr.dataset.pzAssigned=assignmentState(row);let dot=q('.pz-assignment-dot',tr);if(!dot){dot=document.createElement('span');dot.className='pz-assignment-dot';tr.firstElementChild?.prepend(dot);}dot.className=`pz-assignment-dot ${assignmentState(row)}`;
   ...
   let actions=q('.provider-actions',tr)||tr.lastElementChild;if(actions&&!q('[data-pz-assign-button]',tr)){const b=q('[data-provider-assign]',actions)||document.createElement('button'),label=row.assignment?.customer_id?'Projekt zuordnen':'Kunde zuordnen';b.type='button';b.dataset.pzAssignButton='';b.className='secondary subtle pz-assign-mini provider-assign-icon';b.title=label;b.setAttribute('aria-label',label);b.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M10 3a4 4 0 1 0 0 8 4 4 0 0 0 0-8M3 20v-2a7 7 0 0 1 11.6-5.3 6 6 0 0 0-1.4 2.1A5 5 0 0 0 12 18v2zm14-8v3h-3v2h3v3h2v-3h3v-2h-3v-3z"/></svg>';b.onclick=e=>{e.stopPropagation();row.assignment?.customer_id?chooseProject(provider,row):chooseCustomer(provider,row);};actions.append(b);}
  });
