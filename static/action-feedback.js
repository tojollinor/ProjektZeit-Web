...
(()=>{
 ...
 const safe=["/api/v1/customers", "/api/v1/categories", "/api/v1/projects", "/api/v1/users", "/api/v1/profile/avatar", "/api/v1/sessions/disconnect", "/api/v1/integrations/save", "/api/v1/integrations/remove", "/api/v1/account/save", "/api/v1/account/theme"];
 const eligible=path=>safe.includes(path)||(/\/(save|assign(?:\/(?:customer|project))?|profile|phone|contact-phone|contact|device|delete|action|review|reopen|edit|update|add|archive|begin|end|start|stop|pause|resume|active|approve|reject|submit|cancel|pay|close|swap|settings|status|clone|merge|roles|preferences|reset-user)$/.test(path)&&!/^\/api\/v1\/(auth|integrations|archive|provider\/refresh|settings|account)\//.test(path)&&path!=='/api/v1/provider/navigation-status');
 ...
 function end(b,a,ok,message,uncertain=false){if(!a)return;a.error||=!ok;a.uncertain||=uncertain;if(--a.count)return;
  ...
  if(a.error&&b.isConnected){let note=b.parentElement.querySelector('[data-action-error]');if(!note){note=document.createElement('p');note.dataset.actionError='';note.setAttribute('role','alert');b.after(note);}note.textContent=message||'Speichern fehlgeschlagen. Eingaben bleiben erhalten.';}
 ...
 window.fetch=async(input,init={})=>{
  ...
  const b=activation?.button||document.activeElement?.closest('button[type=submit]'),a=begin(b,activation?.disabled),headers=new Headers(init.headers||{}),key=headers.get('Idempotency-Key')||crypto.randomUUID();headers.set('Idempotency-Key',key);
  let response;
