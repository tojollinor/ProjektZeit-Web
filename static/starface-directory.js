(()=>{
 ...
 function setupTickets(box,id,data){
  ...
  form.onsubmit=async event=>{event.preventDefault();const option=select.selectedOptions[0];if(!option?.value){window.pzUI.message('Eingabe prüfen','Bitte eine Zammad-Organisation auswählen.');return;}await post('/api/v1/customers/zammad-organization',{customer_id:id,organization_id:option.value,name:option.dataset.name||option.textContent});openCustomer(id);};
 ...
 async function enhanceCustomer(id){
  ...
  addDeviceSearch(box);
  try{const data=await loadCustomerDetail(id);if(box.dataset.customerId!==String(id))return;setupTickets(box,id,data);await loadDirectory(false);decorateHistory(box);}catch(_){decorateHistory(box);}
 }
