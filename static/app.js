...
const api=async(path,options={})=>{options.headers={"Content-Type":"application/json",...(state.csrf?{"X-CSRF-Token":state.csrf}:{}),...(options.headers||{})};const r=await fetch(path,options);const j=await r.json();if(!r.ok)throw new Error(j.error||"Unbekannter Fehler");if(options.method==='POST')window.pzNotifyMutation(path);return j};
