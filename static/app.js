...
$("#login-form").addEventListener("submit",async e=>{e.preventDefault();$("#login-error").textContent="";const form=e.currentTarget;try{const result=await post("/api/v1/login",{username:$("#login-user").value,password:$("#login-password").value,otp:form.querySelector('[name=otp]')?.value||'',enroll_2fa:!!form.querySelector('[name=enroll_2fa]')?.checked||!!form.querySelector('[data-enrollment]')});if(result.mfa_required){let box=form.querySelector('[data-mfa]');if(!box){box=document.createElement('div');box.dataset.mfa='';form.querySelector('button').before(box);}box.innerHTML=(result.enrollment_required?'<p data-enrollment>2FA einrichten: Diesen Schlüssel in einer Authenticator-App als zeitbasiertes Konto hinzufügen:</p><code class="preserve-lines">'+window.pzUI.h(result.secret)+'</code>':'<p>Authenticator-Code oder Wiederherstellungscode eingeben.</p>')+'<label>2FA-Code<input name="otp" autocomplete="one-time-code" required></label><button type="button" class="secondary" data-restart-login>Anmeldung neu beginnen</button>';box.querySelector('[data-restart-login]').onclick=()=>box.remove();box.querySelector('input').focus();return;}if(result.recovery_codes)await window.pzUI.message('Wiederherstellungscodes sichern','Diese Codes jeweils nur einmal verwenden. Sicher aufbewahren:\n'+result.recovery_codes.join('\n'));form.querySelector('[data-mfa]')?.remove();await boot();}catch(error){$("#login-error").textContent=error.message;}});
const enrollLabel=document.createElement('label');enrollLabel.className='mfa-option';enrollLabel.innerHTML='<input type="checkbox" name="enroll_2fa"> Authenticator einrichten (optional)';$('#login-form button').before(enrollLabel);
for(const field of ['#login-user','#login-password'])$(field).addEventListener('input',()=>$('#login-form [data-mfa]')?.remove());
