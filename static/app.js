...
async function boot(){
 let me;try{me=await api('/api/v1/me');}catch(error){state.user=null;$('#login').classList.remove('hidden');$('#app').classList.add('hidden');return;}
 ...
 try{
  ...
  if(result.mfa_required){
   ...
   if(result.method_selection){
    delete form.dataset.mfaMethod;box.innerHTML='<p><strong>Zweiten Faktor auswählen</strong><br>Diese Auswahl wird für dein Konto gespeichert.</p><div class="mfa-methods"><button type="button" class="secondary" data-mfa-select="totp">Authenticator-App (TOTP)</button>'+(result.email_available?'<button type="button" class="secondary" data-mfa-select="email">Code per E-Mail</button>':'')+'</div>'+(result.email_unavailable_reason?'<p class="muted">E-Mail-Code nicht verfügbar: '+esc(result.email_unavailable_reason)+'</p>':'')+'<button type="button" class="secondary subtle" data-restart-login>Anmeldung neu beginnen</button>';
