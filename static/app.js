...
window.pzNotifyMutation=path=>{if(/\/(save|assign(?:\/(?:customer|project))?|profile|phone|contact-phone|contact|device|delete|action|review|reopen|edit|update|add|archive|begin|end|start|stop|pause|resume|active)$/.test(path)||path==='/api/v1/projects/status')document.dispatchEvent(new CustomEvent('pz-data-changed',{detail:{path}}));};
const api=async(path,options={})=>{options.headers={"Content-Type":"application/json",...(state.csrf?{"X-CSRF-Token":state.csrf}:{}),...(options.headers||{})};const r=await fetch(path,options);let j;try{j=await r.json();}catch(_){throw new Error(r.status>=500?"Der Server ist momentan nicht erreichbar. Bitte erneut versuchen.":"Der Server hat keine gültige Antwort geliefert. Bitte erneut laden.");}if(!r.ok)throw new Error(j.error||"Anfrage fehlgeschlagen ("+r.status+")");if(options.method==='POST')window.pzNotifyMutation(path);return j};
...
async function boot(){
 let me;try{me=await api('/api/v1/me');}catch(error){state.user=null;$('#login').classList.remove('hidden');$('#app').classList.add('hidden');if(location.pathname!=='/login'&&location.pathname!=='/client/authorize'){const target=location.pathname+location.search+location.hash;history.replaceState({},'',`/login?return=${encodeURIComponent(target)}`);}return;}
 state.user=me.user;state.csrf=me.csrf;if(me.user.password_change_required){showForcedPasswordChange();return;}$('#login').classList.add('hidden');$('#app').classList.remove('hidden');$('#username').textContent=me.user.username;$('#avatar').textContent=me.user.username[0].toUpperCase();$('#role').textContent=me.user.role==='admin'?'Administrator':'Benutzer';$$('.admin-only').forEach(x=>x.style.display=me.user.role==='admin'?'flex':'none');
 document.dispatchEvent(new CustomEvent('pz-auth-ready'));
 try{await refresh();}catch(error){toast(error.message);}
 document.dispatchEvent(new CustomEvent('pz-app-ready'));
}
let dashboardRequest=0;
async function refresh(){const request=++dashboardRequest;const data=await api("/api/v1/dashboard?day="+encodeURIComponent($("#timeline-date")?.value||dateValue(new Date())));if(request!==dashboardRequest)return;state.data=data;render();renderWork()}
function render(){const d=state.data,active=d.entries.find(e=>!e.ended_at);$("#project-count").textContent=d.projects.length;$("#entry-count").textContent=d.entries.length;const today=new Date().toDateString();let seconds=d.entries.filter(e=>e.ended_at&&new Date(e.started_at).toDateString()===today).reduce((n,e)=>n+(new Date(e.ended_at)-new Date(e.started_at))/1000,0);$("#today-total").textContent=duration(new Date(0),new Date((d.today_seconds??seconds)*1000));$("#entry-table").innerHTML=d.entries.length?d.entries.map(e=>`<tr><td><strong>${esc(e.project)}</strong></td><td>${esc(e.customer||"Ohne Kunde")}</td><td>${esc(e.category)}</td><td>${new Date(e.started_at).toLocaleString("de-DE")}</td><td>${duration(e.started_at,e.ended_at)}</td><td><span class="badge ${e.ended_at?'':'live'}">${e.ended_at?'Gespeichert':'Läuft'}</span></td></tr>`).join(""):`<tr><td colspan="6" class="empty">Noch keine Stempelungen vorhanden.</td></tr>`;renderTimeline(d.entries);fillSelect("#project-select",d.projects,"Projekt wählen");fillSelect("#category-select",d.categories,"Kategorie wählen");fillSelect("#project-customer",d.customers.filter(c=>!c.owner_id||c.owner_id===state.user.id),"Ohne Kunde",true);$("#user-list").innerHTML=d.users.map(u=>`<div class="user-row"><span class="mini-avatar">${esc(u.username[0].toUpperCase())}</span><div><strong>${esc(u.username)}</strong><small>${u.role==='admin'?'Administrator':'Benutzer'} · ${u.active?'Aktiv':'Deaktiviert'}</small></div><span class="badge">${u.role}</span></div>`).join("");setTimer(active)}
...
function updateTimelineScale(){
  ...
  const width=Math.max(160,scroll.clientWidth-4)*(state.timelineZoom||1);
...
function fillSelect(selector,items,placeholder,allowEmpty=false){const el=$(selector);if(!el)return;const old=el.value;el.innerHTML=`<option value="">${placeholder}</option>`+items.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join("");if([...el.options].some(o=>o.value===old))el.value=old;if(!allowEmpty&&!el.value&&items[0])el.value=items[0].id;if(/category/.test(selector))window.pzUI?.category(el);}
function setTimer(active){clearInterval(state.tick);const pill=$("#live-pill"),button=$("#timer-button"),timer=$("#timer");pill.classList.toggle("running",!!active&&!active.is_idle);pill.querySelector("span").textContent=active?(active.is_idle?"Kein Projekt läuft":"Projekt läuft"):"Kein Projekt läuft";button.textContent="Ausgewähltes Projekt starten / wechseln";button.classList.remove("stop");button.disabled=!state.data.work;const update=()=>{timer.textContent=active?duration(active.started_at,new Date().toISOString()):"00:00:00";if(state.data.work)$("#work-detail").textContent=`Seit ${new Date(state.data.work.started_at).toLocaleTimeString('de-DE')} · ${duration(state.data.work.started_at,new Date())}`};update();if(active)state.tick=setInterval(update,1000)}
function showView(name){if(name==='settings'||name==='account')name='settings-profile';if(!$('#view-'+name)){toast('Diese Seite ist nicht mehr verfügbar.');return;}$$('.view').forEach(v=>v.classList.remove('active-view'));$("#view-"+name).classList.add('active-view');$$('.nav').forEach(n=>n.classList.toggle('active',n.dataset.view===name));$("#page-title").textContent={dashboard:"Übersicht",tracking:"Zeiterfassung",users:"Benutzerverwaltung",settings:"Einstellungen",zammad:"Zammad",starface:"STARFACE",teamviewer:"TeamViewer"}[name];$(".sidebar").classList.remove("open");document.dispatchEvent(new CustomEvent('pz-view-changed',{detail:{name}}));}
$("#login-form").addEventListener("submit",async e=>{
 e.preventDefault();$("#login-error").textContent="";const form=e.currentTarget,resend=form.dataset.resendMfa==='1';delete form.dataset.resendMfa;
 try{
  const result=await post("/api/v1/login",{username:$("#login-user").value,password:$("#login-password").value,otp:form.querySelector('[name=otp]')?.value||'',mfa_method:form.dataset.mfaMethod||'',resend_mfa:resend,remember:!!form.querySelector('[name=remember]')?.checked});
  if(result.mfa_required){
   let box=form.querySelector('[data-mfa]');if(!box){box=document.createElement('div');box.dataset.mfa='';form.querySelector('button[type=submit]').before(box);}
   if(result.method_selection){
    delete form.dataset.mfaMethod;box.innerHTML='<p><strong>Zweiten Faktor auswählen</strong><br>Diese Auswahl gilt nur für diese Anmeldung.</p><div class="mfa-methods"><button type="button" class="secondary" data-mfa-select="totp">Authenticator-App (TOTP)</button>'+(result.email_available?'<button type="button" class="secondary" data-mfa-select="email">Code per E-Mail</button>':'')+'</div>'+(result.email_unavailable_reason?'<p class="muted">E-Mail-Code nicht verfügbar: '+esc(result.email_unavailable_reason)+'</p>':'')+'<button type="button" class="secondary subtle" data-restart-login>Anmeldung neu beginnen</button>';
    box.querySelectorAll('[data-mfa-select]').forEach(button=>button.onclick=()=>{form.dataset.mfaMethod=button.dataset.mfaSelect;form.requestSubmit();});
   }else if(result.mfa_method==='email'){
    form.dataset.mfaMethod='email';box.innerHTML='<p><strong>E-Mail-Code eingeben</strong><br>Ein Code wurde an '+esc(result.masked_email||'deine E-Mail-Adresse')+' gesendet. Er ist zehn Minuten gültig.</p><label>Anmeldecode<input name="otp" autocomplete="one-time-code" inputmode="text" required></label><div class="mfa-actions"><button type="button" class="secondary" data-resend-mfa>Code erneut senden</button><button type="button" class="secondary subtle" data-restart-login>Anmeldung neu beginnen</button></div>';
    const resendButton=box.querySelector('[data-resend-mfa]');if(result.retry_after){resendButton.disabled=true;let left=Number(result.retry_after);const timer=setInterval(()=>{left--;if(!resendButton.isConnected||left<=0){clearInterval(timer);if(resendButton.isConnected){resendButton.disabled=false;resendButton.textContent='Code erneut senden';}}else resendButton.textContent='Erneut senden ('+left+' s)';},1000);}
    resendButton.onclick=()=>{form.dataset.resendMfa='1';box.querySelector('[name=otp]').value='';form.requestSubmit();};
   }else{
    form.dataset.mfaMethod='totp';box.innerHTML=(result.enrollment_required?'<p><strong>Authenticator-App (TOTP) einrichten</strong><br>Diesen Schlüssel als zeitbasiertes Konto hinzufügen:</p><code class="preserve-lines">'+esc(result.secret)+'</code>':'<p>TOTP-Code aus der Authenticator-App oder Wiederherstellungscode eingeben.</p>')+'<label>2FA-Code<input name="otp" autocomplete="one-time-code" inputmode="numeric" required></label><button type="button" class="secondary subtle" data-restart-login>Anmeldung neu beginnen</button>';
   }
   const restart=box.querySelector('[data-restart-login]');if(restart)restart.onclick=()=>{box.remove();delete form.dataset.mfaMethod;delete form.dataset.resendMfa;};
   box.querySelector('input')?.focus();return;
  }
  if(result.recovery_codes)await window.pzUI.message('Wiederherstellungscodes sichern','Diese Codes jeweils nur einmal verwenden. Sicher aufbewahren:\n'+result.recovery_codes.join('\n'));
  form.querySelector('[data-mfa]')?.remove();delete form.dataset.mfaMethod;await boot();
 }catch(error){$("#login-error").textContent=error.message;}
});
for(const field of ['#login-user','#login-password'])$(field).addEventListener('input',()=>{const form=$('#login-form');form.querySelector('[data-mfa]')?.remove();delete form.dataset.mfaMethod;});

const resetLink=document.createElement('button');resetLink.type='button';resetLink.className='login-link';resetLink.textContent='Passwort vergessen?';resetLink.hidden=true;$('#login-form').after(resetLink);
post('/api/v1/auth/email/options',{}).then(options=>{resetLink.hidden=!options.password_reset_available;if(options.remember_login_allowed&&!$('#login-form [name=remember]')){const label=document.createElement('label');label.className='login-remember';label.innerHTML='<input type="checkbox" name="remember"> Angemeldet bleiben';$('#login-form button[type=submit]').before(label);}}).catch(()=>{});
resetLink.onclick=()=>window.pzUI.formDialog('Passwort zurücksetzen','<p>Wir senden einen einmalig verwendbaren Link an die hinterlegte Adresse.</p><label>E-Mail-Adresse<input type="email" name="email" autocomplete="email" required></label>',async values=>{const result=await post('/api/v1/auth/password-reset/request',values);await window.pzUI.message('E-Mail angefordert',result.message);});

async function handleEmailActionLinks(){
 const url=new URL(location.href),verify=url.searchParams.get('verify-email'),reset=url.searchParams.get('password-reset');if(!verify&&!reset)return;
 const clean=()=>{url.searchParams.delete('verify-email');url.searchParams.delete('password-reset');history.replaceState({},'',url.pathname+(url.search?'?'+url.searchParams.toString():'')+url.hash);};
 if(verify){try{await post('/api/v1/auth/email-verification/complete',{token:verify});clean();await window.pzUI.message('E-Mail bestätigt','Deine E-Mail-Adresse wurde bestätigt. Du kannst dich jetzt anmelden.');}catch(error){clean();await window.pzUI.message('Bestätigung nicht möglich',error.message);}return;}
 window.pzUI.formDialog('Neues Passwort festlegen','<label>Neues Passwort<input type="password" name="password" minlength="12" autocomplete="new-password" required></label><label>Passwort wiederholen<input type="password" name="confirmation" minlength="12" autocomplete="new-password" required></label>',async values=>{await post('/api/v1/auth/password-reset/complete',{token:reset,...values});clean();await window.pzUI.message('Passwort geändert','Das neue Passwort wurde gespeichert. Du kannst dich jetzt anmelden.');});
}
handleEmailActionLinks();

function showForcedPasswordChange(){
 $('#app').classList.add('hidden');$('#login').classList.remove('hidden');$('#login-form').classList.add('hidden');resetLink.hidden=true;let box=$('[data-forced-password]');if(!box){box=document.createElement('div');box.dataset.forcedPassword='';box.innerHTML='<p><strong>Vorläufiges Passwort ändern</strong></p><p class="muted">Lege vor der weiteren Nutzung ein eigenes Passwort fest.</p><form><label>Aktuelles Passwort<input type="password" name="current_password" autocomplete="current-password" required></label><label>Neues Passwort<input type="password" name="password" minlength="12" autocomplete="new-password" required></label><label>Neues Passwort wiederholen<input type="password" name="confirmation" minlength="12" autocomplete="new-password" required></label><div class="actions"><button class="primary">Passwort speichern</button><button type="button" class="secondary" data-forced-logout>Abmelden</button></div><p class="error" role="alert"></p></form>';$('.login-card').append(box);const form=box.querySelector('form');form.querySelector('[data-forced-logout]').onclick=async()=>{await post('/api/v1/logout');location.reload();};form.onsubmit=async event=>{event.preventDefault();const error=form.querySelector('.error');error.textContent='';try{await post('/api/v1/account/password',Object.fromEntries(new FormData(form)));box.remove();$('#login-form').classList.remove('hidden');await boot();}catch(reason){error.textContent=reason.message;}};}box.querySelector('[name=current_password]')?.focus();
}

$("#logout").addEventListener("click",async()=>{await post("/api/v1/logout");location.reload()});
$("#timeline-date").value=dateValue(new Date());$("#timeline-date").addEventListener("change",()=>refresh().catch(e=>toast(e.message)));
...
function moveDay(delta){const value=new Date($("#timeline-date").value+"T12:00:00");value.setDate(value.getDate()+delta);$("#timeline-date").value=dateValue(value);refresh().catch(e=>toast(e.message))}
$("#day-prev").addEventListener("click",()=>moveDay(-1));$("#day-next").addEventListener("click",()=>moveDay(1));$("#day-today").addEventListener("click",()=>{$("#timeline-date").value=dateValue(new Date());refresh().catch(e=>toast(e.message))});
...
form("#project-form","/api/v1/projects",()=>({name:$("#project-name").value,customer_id:+$("#project-customer").value||null}));form("#user-form","/api/v1/users",()=>({username:$("#new-username").value,password:$("#new-password").value,role:$("#new-role").value}));
...
function renderWork(){
  ...
  const picker=$("#quick-projects"),oldCustomer=picker.querySelector('[data-work-customer]')?.value,oldProject=picker.querySelector('[data-work-project]')?.value;
  const customers=d.customers.filter(c=>active.some(p=>p.customer_id===c.id));
  picker.innerHTML='<div class="work-project-picker"><label>Kunde mit aktiven Projekten<select data-work-customer><option value="">Kunde wählen …</option>'+customers.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')+(active.some(p=>!p.customer_id)?'<option value="none">Ohne Kunde</option>':'')+'</select></label><label>Projekt<select data-work-project></select></label><label>Zeitkategorie<select data-work-category></select></label><button type="button" class="primary" data-work-switch>Projekt starten / wechseln</button></div>';
  const customer=picker.querySelector('[data-work-customer]'),project=picker.querySelector('[data-work-project]'),category=picker.querySelector('[data-work-category]'),button=picker.querySelector('[data-work-switch]');
  customer.value=oldCustomer||(running&&!running.is_idle?String(active.find(p=>p.id===running.project_id)?.customer_id||'none'):'');
  function choose(){project.innerHTML='<option value="">Projekt wählen …</option>'+active.filter(p=>customer.value&&(customer.value==='none'?!p.customer_id:String(p.customer_id)===customer.value)).map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');if(oldProject)project.value=oldProject;if(!project.value&&project.options.length>1)project.selectedIndex=1;button.disabled=!working||!project.value;}
  category.innerHTML=d.categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('');category.value=$('#category-select').value;window.pzUI?.category(category);
  ...
  $("#entry-table").innerHTML=d.entries.map(e=>`<tr><td><strong class="${e.is_idle?'idle-label':''}">${esc(e.project)}</strong><small class="entry-note">${esc(e.note)}</small></td><td>${esc(e.customer||'Ohne Kunde')}</td><td>${esc(e.category)}</td><td>${new Date(e.started_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'})}<small class="entry-end">${e.ended_at?new Date(e.ended_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'}):'Läuft'}</small></td><td>${duration(e.started_at,e.ended_at)}</td><td class="entry-actions"><button class="secondary subtle icon-action" data-edit="${e.id}" aria-label="Stempelung bearbeiten" title="Bearbeiten"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 16.5V20h3.5L18.1 9.4l-3.5-3.5L4 16.5Zm16.7-9.7a1 1 0 0 0 0-1.4l-2.1-2.1a1 1 0 0 0-1.4 0l-1.6 1.6 3.5 3.5 1.6-1.6Z"/></svg></button></td></tr>`).join('')||'<tr><td colspan="6">Noch keine Stempelungen.</td></tr>';
}
...
async function loadIntegrations(){
  ...
  try{
    ...
    container.innerHTML=data.integrations.map(row=>{
      ...
      if(p==='starface')return renderStarfaceCard(row);
      const captureHelp=p==='teamviewer'?`<section class="teamviewer-capture-help"><div><strong>Verbindungserfassung prüfen</strong><p>Wenn keine neuen Verbindungen erscheinen, in TeamViewer <b>Einstellungen → Verbindungsprotokolle</b> öffnen und die Erfassung ausgehender Verbindungen aktivieren.</p></div><ul><li>Verbindungen mit dem angemeldeten Firmenkonto starten</li><li>Im Verlauf Benutzer-, Gruppen- und Datumsfilter entfernen</li><li>Zugehörigkeit zum bisherigen Unternehmensprofil prüfen</li></ul><p>Die API kann Konto und Berichte prüfen, diesen persönlichen Schalter aber nicht selbst ändern. <a href="https://www.teamviewer.com/en-us/global/support/knowledge-base/teamviewer-remote/security/connection-reports/" target="_blank" rel="noopener noreferrer">TeamViewer-Anleitung</a></p></section>`:'';
      return `<article class="panel integration-card" data-provider="${p}"><div class="panel-head"><h3>${info.name}</h3><span class="badge">${row.has_secret?'Gespeichert':'Noch nicht eingerichtet'}</span></div><p class="integration-note">${info.note} <a href="${info.docs}" target="_blank" rel="noopener noreferrer">API-Dokumentation</a></p>${captureHelp}<div class="integration-fields"><label>Domain<input data-field="domain" value="${attr(row.domain)}" placeholder="${info.placeholder}" autocomplete="off" spellcheck="false"></label><label>${p==='starface'?'Login-ID':'Benutzername'+(p==='teamviewer'?' (optional)':'')}<input data-field="username" value="${attr(row.username)}" maxlength="250" autocomplete="off" spellcheck="false"></label><label>${info.label}<input data-field="secret" type="password" maxlength="4096" autocomplete="new-password" placeholder="${row.has_secret?'Gespeichert – leer lassen zum Beibehalten':info.label+' eingeben'}"></label></div><div class="panel-actions"><button type="button" class="primary" data-integration-action="save">Speichern</button><button type="button" class="secondary" data-integration-action="test">${p==='teamviewer'?'Berechtigungen &amp; Protokolle prüfen':'Debug · Verbindung testen'}</button><button type="button" class="secondary subtle" data-integration-action="remove">Verknüpfung entfernen</button></div><div class="integration-status" role="status"></div><div class="debug-output hidden"></div></article>`;
    }).join('');
...
$('#integration-cards').addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='test'){
      const tv=result.teamviewer;
      status.textContent=provider==='teamviewer'&&result.ok&&result.attention?'TeamViewer ist erreichbar, die Diagnose benötigt aber Aufmerksamkeit. Details siehe unten.':result.ok?'Verbindungstest erfolgreich. Bitte Datenprobe und Hinweise prüfen.':'Verbindungstest unvollständig oder fehlgeschlagen. Details siehe unten.';
      status.classList.add(result.ok?(result.attention?'warning':'success'):'failure');
      output.classList.remove('hidden');
      const hasSample=(result.steps||[]).some(step=>step.ok&&step.count!==undefined&&(provider!=='teamviewer'||step.name.startsWith('Verbindungsberichte')));const checks=document.createElement('p');checks.className='integration-note';checks.textContent=hasSample?'Relevante Felder in der Datenprobe: '+(result.checks||[]).map(check=>`${check.name}: ${check.found?'vorhanden':'nicht nachgewiesen'}`).join(' · '):'Datenfelder noch nicht geprüft: Es konnte keine verwertbare Datenprobe abgerufen werden.';
      const account=tv?.account||{},tvSummary=tv?`<section class="teamviewer-diagnostic-summary ${result.attention?'needs-attention':'healthy'}"><div><small>API-DIAGNOSE</small><strong>${tv.state==='recent_reports'?'Aktuelle Verbindungen gefunden':tv.state==='no_recent_reports'?'Keine Verbindung in den letzten '+tv.recent_window_days+' Tagen':tv.state==='reports_permission_missing'?'Berichtsrecht fehlt':'Prüfung nicht vollständig'}</strong></div><dl><div><dt>Konto</dt><dd>${esc(account.name||account.email||(tv.account_verified?'Bestätigt':'Nicht prüfbar'))}</dd></div><div><dt>Unternehmen</dt><dd>${esc(account.company_name||'Nicht nachgewiesen')}</dd></div><div><dt>Lizenz</dt><dd>${esc(account.license||'Nicht nachgewiesen')}</dd></div><div><dt>Neue Berichte</dt><dd>${Number(tv.recent_connections||0)}${tv.latest_connection_at?' · zuletzt '+esc(new Date(tv.latest_connection_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'})):''}</dd></div></dl><p><strong>Manueller Restschritt:</strong> ${esc(tv.settings_path)}. Der persönliche Schalter ist nicht Bestandteil der öffentlichen API und kann von ProjektZeit deshalb weder ausgelesen noch geändert werden.</p>${!tv.account_verified?'<p>Für den Konto-/Unternehmensabgleich einen neuen Script-Token mit Leserechten für <b>Kontoinformationen</b> und <b>Verbindungsberichte</b> erstellen. Bestehende TeamViewer-Tokens lassen sich nicht erweitern.</p>':''}</section>`:'';
      output.innerHTML=tvSummary+`<p class="integration-note">${esc(result.note)}</p><p class="integration-meta">${esc(new Date(result.checked_at).toLocaleString('de-DE',{timeZone:'Europe/Berlin'}))} · ${result.duration_ms} ms · Test speichert keine Daten</p>`+result.steps.map(step=>`<section class="debug-step ${!step.ok&&step.required===false?'advisory':''}"><strong>${step.ok?'✓':step.required===false?'!':'✕'} ${esc(step.name)}${step.status?' · HTTP '+step.status:''}</strong><p class="step-path">${esc(step.path||'')} ${step.duration_ms!==undefined?'· '+step.duration_ms+' ms':''}</p><p>${esc(step.message)}</p>${step.count!==undefined?`<p>${step.count} Datensätze in der Antwort · Vorschau maximal 5</p><p>Felder: ${esc((step.fields||[]).join(', ')||'Keine Datensatzfelder geliefert')}</p><details open><summary>Bereinigte Datenvorschau</summary><pre>${esc(JSON.stringify(step.preview,null,2))}</pre></details>`:''}</section>`).join('');
...
let customerRefreshPending=null;document.addEventListener('pz-data-changed',e=>{if(!state.user||!/^\/api\/v1\/customers(?:\/|$)/.test(e.detail?.path||''))return;if(customerRefreshPending)return;customerRefreshPending=refresh().catch(error=>window.pzDiagnostics?.record?.('customer-refresh-error',{message:error.message})).finally(()=>customerRefreshPending=null);});
