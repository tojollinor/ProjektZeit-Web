...
const api=async(path,options={})=>{options.headers={"Content-Type":"application/json",...(state.csrf?{"X-CSRF-Token":state.csrf}:{}),...(options.headers||{})};const r=await fetch(path,options);const j=await r.json();if(!r.ok)throw new Error(j.error||"Unbekannter Fehler");if(options.method==='POST'&&(/\/(save|assign(?:\/customer)?|profile|phone|contact-phone|contact|device|delete|action|review|reopen|edit|begin|end|start|stop|pause|resume|active)$/.test(path)||path==='/api/v1/projects/status'))document.dispatchEvent(new CustomEvent("pz-data-changed",{detail:{path}}));return j};
...
async function refresh(){state.data=await api("/api/v1/dashboard?day="+encodeURIComponent($("#timeline-date")?.value||dateValue(new Date())));render();renderWork()}
