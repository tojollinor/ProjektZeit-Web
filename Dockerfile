...
CMD ["python", "-u", "-c", "import app, customer_runtime, feature_runtime, contact_runtime, ux_runtime, provider_cache_runtime, api_runtime, zammad_cache_runtime, provider_nav_runtime, worktime_runtime, customer_extra_runtime, next_batch_runtime, frontend_update_runtime, final_batch_runtime, final_batch_fix_runtime, profile_avatar_runtime; customer_runtime.install(app); feature_runtime.install(app); contact_runtime.install(app); ux_runtime.install(app); provider_cache_runtime.install(app); api_runtime.install(app); zammad_cache_runtime.install(app); provider_nav_runtime.install(app); worktime_runtime.install(app); customer_extra_runtime.install(app); next_batch_runtime.install(app); frontend_update_runtime.install(app); final_batch_runtime.install(app); final_batch_fix_runtime.install(app); profile_avatar_runtime.install(app); app.init_db(); print('ProjektZeit Web läuft auf http://%s:%d' % (app.HOST, app.PORT)); app.ThreadingHTTPServer((app.HOST, app.PORT), app.App).serve_forever()"]
