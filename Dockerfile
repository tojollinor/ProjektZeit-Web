...
CMD ["python", "-u", "-c", "import app, customer_runtime, feature_runtime, contact_runtime, ux_runtime, provider_cache_runtime, api_runtime, zammad_cache_runtime; customer_runtime.install(app); feature_runtime.install(app); contact_runtime.install(app); ux_runtime.install(app); provider_cache_runtime.install(app); api_runtime.install(app); zammad_cache_runtime.install(app); app.init_db(); print('ProjektZeit Web läuft auf http://%s:%d' % (app.HOST, app.PORT)); app.ThreadingHTTPServer((app.HOST, app.PORT), app.App).serve_forever()"]
