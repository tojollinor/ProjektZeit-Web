...
permission dependencies and super-admin migration.
"""
import hashlib
import json
import re
...
DEPENDENCIES = {
    'users.create': {'users.view','admin.options.view'},
    'users.edit': {'users.view','admin.options.view'},
    'users.disable': {'users.view','admin.options.view'},
    'users.roles.assign': {'users.view','roles.view','admin.options.view'},
    'roles.view': {'admin.options.view'}, 'roles.create': {'roles.view','admin.options.view'},
    'roles.edit': {'roles.view','admin.options.view'}, 'roles.clone': {'roles.view','admin.options.view'},
    'roles.delete': {'roles.view','admin.options.view'}, 'roles.reset_user': {'roles.view','admin.options.view'},
    ...
    'notifications.edit': {'admin.options.view'},
    'integrations.edit': {'integrations.view','admin.options.view'}, 'integrations.sync': {'integrations.view'},
    'integrations.debug': {'integrations.view','admin.options.view'}, 'system.options.edit': {'admin.options.view'},
...
def _register_permissions():
    # Keep customers.view as compatibility alias while exposing useful finer-grained rights.
    categories={
      'Kunden': [
        ('customers.view_basic','Kunden-Grunddaten ansehen'),('customers.view_contacts','Kontaktdaten ansehen'),
...
def migrate(c):
    ...
    CREATE INDEX IF NOT EXISTS idx_customer_provider_links_customer ON customer_provider_links(owner_id,customer_id,provider);
    ...
    ''')
    cols=_columns(c,'customers')
    if 'archived' not in cols:c.execute("ALTER TABLE customers ADD COLUMN archived INTEGER NOT NULL DEFAULT 0")
    # Migrate the legacy super-admin into the ordinary Admin role before removing the bypass.
    ...
    if admin:
        ...
        if super_role:
            legacy += [r['user_id'] for r in c.execute('SELECT user_id FROM user_role_links WHERE role_id=?',(super_role['id'],))]
        for uid in set(legacy):
            c.execute('INSERT OR IGNORE INTO user_role_links(user_id,role_id) VALUES(?,?)',(uid,admin['id']))
    ...
    # Normalize all saved role permission sets and give the system Admin the intended broad rights.
    for role in list(c.execute('SELECT id,role_key FROM role_definitions')):
        ...
        perms=closure(perms)
        c.execute('DELETE FROM role_permissions WHERE role_id=?',(role['id'],))
...
def _provider_identifier(provider,raw,hint=None):
    ...
    if provider=='teamviewer':
        value=raw.get('deviceid') or raw.get('device_id') or hint.get('device_id')
        return ('teamviewer_id',str(value).strip()) if value not in (None,'') else ('','')
    if provider=='starface':
        direction=str(raw.get('direction') or '').upper()
        value=raw.get('callerNumber') if direction=='INBOUND' else raw.get('calledNumber')
        return ('phone',str(value or '').strip())
    if provider=='zammad':
        customer=raw.get('customer') if isinstance(raw.get('customer'),dict) else {}
        value=customer.get('email') or raw.get('customer_email') or hint.get('email')
...
def _ensure_assignment(c,uid,provider,event):
    ...
    customer_id=None
    if val:
        link=c.execute('SELECT customer_id FROM customer_provider_links WHERE owner_id=? AND provider=? AND link_type=? AND link_value=?',(uid,provider,typ,val)).fetchone()
        if link:customer_id=link['customer_id']
    if customer_id:
...
def customer_links(c,uid,cid):
    return [dict(r) for r in c.execute('SELECT id,provider,link_type,link_value,display_name,created_at FROM customer_provider_links WHERE owner_id=? AND customer_id=? ORDER BY provider,display_name,link_value',(uid,cid))]
...
def _bulk_assign(c,uid,provider,customer_id,link_type,link_value,actor):
    count=0
    for event in c.execute('SELECT external_key,raw_json,hint_json FROM provider_events WHERE owner_id=? AND provider=?',(uid,provider)):
        try:raw=json.loads(event['raw_json'] or '{}');hint=json.loads(event['hint_json'] or '{}')
        except Exception:continue
        typ,val=_provider_identifier(provider,raw,hint)
        if typ==link_type and val==link_value:
            c.execute('DELETE FROM provider_assignments WHERE owner_id=? AND provider=? AND external_key=?',(uid,provider,event['external_key']))
            c.execute('INSERT INTO provider_assignments(owner_id,provider,external_key,customer_id,project_id,match_type,match_value,assigned_by,assigned_at) VALUES(?,?,?,?,NULL,?,?,?,?,?)',())
    return count


def install(app):
    _register_permissions()
    # Remove special super-admin behavior globally. Existing callers now use ordinary role permissions.
    admin_controls.is_superadmin=lambda c,uid:False
    def permissions_for_user(c,uid):
        return {r['permission_key'] for r in c.execute('SELECT rp.permission_key FROM role_permissions rp JOIN user_role_links ur ON ur.role_id=rp.role_id WHERE ur.user_id=?',(uid,))}
    admin_controls.permissions_for_user=permissions_for_user
    admin_controls.can=lambda c,uid,p:p in permissions_for_user(c,uid)
    old_set=admin_controls._set_role_permissions
    admin_controls._set_role_permissions=lambda c,rid,perms:old_set(c,rid,closure(perms))
    ...
    # Enrich provider list rows with stable customer/project assignment state.
    try:
        pc=__import__('provider_cache_runtime');old_cached=pc.cached_list
        def cached(c,uid,provider,body):return enrich_rows(c,uid,provider,old_cached(c,uid,provider,body))
        pc.cached_list=cached
    except Exception:pass
    try:
        zc=__import__('zammad_cache_runtime');old_z=zc.cached_list
        def zcached(c,uid):return enrich_rows(c,uid,'zammad',old_z(c,uid))
        zc.cached_list=zcached
    except Exception:pass

    # Hide manual callbacks from the outstanding-call widget.
    try:
        nb=__import__('next_batch_runtime');old_missed=nb._missed_calls
        def missed(c,uid):
            hidden={r['external_key'] for r in c.execute('SELECT external_key FROM starface_manual_callbacks WHERE owner_id=?',(uid,))}
            return [r for r in old_missed(c,uid) if r.get('external_key') not in hidden]
        nb._missed_calls=missed
    except Exception:pass

    # Personal API tokens only. Effective scopes are intersected with current user rights on every request.
    try:
        ar=__import__('api_runtime')
        old_scopes=ar.available_scopes
        ...
        ar.available_scopes=scopes
        old_auth=ar._api_auth
    ...
    handled={
      '/api/v1/history/object','/api/v1/customers/links','/api/v1/customers/link/add','/api/v1/customers/link/delete',
      '/api/v1/provider/assign/customer','/api/v1/provider/assign/project','/api/v1/starface/callback/manual',
      '/api/v1/customers/archive','/api/v1/customers/delete','/api/v1/admin/api/tokens/list','/api/v1/admin/api/tokens/create','/api/v1/admin/api/tokens/revoke'
    }
    def do_POST(self):
        ...
        try:
            with app.db() as c:
                ...
                if path=='/api/v1/customers/links':
                    admin_controls.require_permission(c,uid,'customers.view_links');cid=int(body.get('customer_id'));_valid_customer(c,uid,cid)
                    return self.send_json(200,{'links':customer_links(c,uid,cid)})
                if path=='/api/v1/customers/link/add':
                    admin_controls.require_permission(c,uid,'customers.edit');cid=int(body.get('customer_id'));_valid_customer(c,uid,cid)
                    ...
                    previous=c.execute('SELECT customer_id FROM customer_provider_links WHERE owner_id=? AND provider=? AND link_type=? AND link_value=?',(uid,provider,typ,val)).fetchone()
                    c.execute('DELETE FROM customer_provider_links WHERE owner_id=? AND provider=? AND link_type=? AND link_value=?',(uid,provider,typ,val))
                    cur=c.execute('INSERT INTO customer_provider_links(owner_id,customer_id,provider,link_type,link_value,display_name,created_by,created_at) VALUES(?,?,?,?,?,?,?,?)',(uid,cid,provider,typ,val,display,uid,now_iso()))
                    matched=0
                    for event in list(c.execute('SELECT external_key,raw_json,hint_json FROM provider_events WHERE owner_id=? AND provider=?',(uid,provider))):
                        ...
                        et,ev=_provider_identifier(provider,raw,hint)
                        if et==typ and ev==val:
                            old=c.execute('SELECT project_id FROM provider_assignments WHERE owner_id=? AND provider=? AND external_key=?',(uid,provider,event['external_key'])).fetchone();project_id=old['project_id'] if old else None
                            c.execute('DELETE FROM provider_assignments WHERE owner_id=? AND provider=? AND external_key=?',(uid,provider,event['external_key']))
                            c.execute('INSERT INTO provider_assignments(owner_id,provider,external_key,customer_id,project_id,match_type,match_value,assigned_by,assigned_at) VALUES(?,?,?,?,?,?,?,?,?)',(uid,provider,event['external_key'],cid,project_id,typ,val,uid,now_iso()));matched+=1
                    _audit(c,uid,'customer',cid,'provider_link_added',{'provider':provider,'type':typ,'value':val,'matched':matched,'moved_from_customer':previous['customer_id'] if previous else None})
                    return self.send_json(200,{'ok':True,'id':cur.lastrowid,'matched':matched})
                if path=='/api/v1/customers/link/delete':
                    admin_controls.require_permission(c,uid,'customers.edit');lid=int(body.get('id'));row=c.execute('SELECT * FROM customer_provider_links WHERE id=? AND owner_id=?',(lid,uid)).fetchone()
                    if not row:raise ValueError('Verknüpfung nicht gefunden.')
                    c.execute('DELETE FROM customer_provider_links WHERE id=?',(lid,));c.execute('UPDATE provider_assignments SET customer_id=NULL,project_id=NULL,assigned_by=?,assigned_at=? WHERE owner_id=? AND provider=? AND match_type=? AND match_value=?',(uid,now_iso(),uid,row['provider'],row['link_type'],row['link_value']))
                    _audit(c,uid,'customer',row['customer_id'],'provider_link_removed',{'provider':row['provider'],'type':row['link_type'],'value':row['link_value']})
                    return self.send_json(200,{'ok':True})
                if path=='/api/v1/provider/assign/customer':
                    provider=str(body.get('provider') or '').lower();key=str(body.get('external_key') or '');cid=int(body.get('customer_id'));_valid_customer(c,uid,cid)
                    ...
                    c.execute('DELETE FROM customer_provider_links WHERE owner_id=? AND provider=? AND link_type=? AND link_value=?',(uid,provider,typ,val));c.execute('INSERT INTO customer_provider_links(owner_id,customer_id,provider,link_type,link_value,display_name,created_by,created_at) VALUES(?,?,?,?,?,?,?,?)',(uid,cid,provider,typ,val,display,uid,now_iso()))
                    matched=0
                    for event in list(c.execute('SELECT external_key,raw_json,hint_json FROM provider_events WHERE owner_id=? AND provider=?',(uid,provider))):
                        try:eraw=json.loads(event['raw_json'] or '{}');eh=json.loads(event['hint_json'] or '{}')
                        except Exception:continue
                        et,evv=_provider_identifier(provider,eraw,eh)
                    ...
                    _audit(c,uid,'customer',cid,'provider_assigned',{'provider':provider,'type':typ,'value':val,'matched':matched})
                ...
                if path=='/api/v1/provider/assign/project':
                    ...
                    c.execute('UPDATE provider_assignments SET project_id=?,assigned_by=?,assigned_at=? WHERE owner_id=? AND provider=? AND external_key=?',(pid,uid,now_iso(),uid,provider,key));_audit(c,uid,'project',pid,'provider_entry_assigned',{'provider':provider,'external_key':key})
                    return self.send_json(200,{'ok':True})
                if path=='/api/v1/starface/callback/manual':
                    ...
                    c.execute('DELETE FROM starface_manual_callbacks WHERE owner_id=? AND external_key=?',(uid,key));c.execute('INSERT INTO starface_manual_callbacks(owner_id,external_key,marked_by,marked_at) VALUES(?,?,?,?)',(uid,key,uid,now_iso()));_audit(c,uid,'starface_call',key,'manually_called_back',{})
                    return self.send_json(200,{'ok':True})
                if path=='/api/v1/customers/archive':
                    admin_controls.require_permission(c,uid,'customers.archive');cid=int(body.get('customer_id'));row=_valid_customer(c,uid,cid);state=1 if body.get('archived',True) else 0;c.execute('UPDATE customers SET archived=? WHERE id=? AND owner_id=?',(state,cid,uid));_audit(c,uid,'customer',cid,'archived' if state else 'restored',{})
                    return self.send_json(200,{'ok':True,'archived':bool(state)})
                if path=='/api/v1/customers/delete':
                    admin_controls.require_permission(c,uid,'customers.delete');cid=int(body.get('customer_id'));row=_valid_customer(c,uid,cid)
                    c.execute('UPDATE provider_assignments SET customer_id=NULL,project_id=NULL,assigned_by=?,assigned_at=? WHERE owner_id=? AND customer_id=?',(uid,now_iso(),uid,cid));c.execute('DELETE FROM customer_provider_links WHERE owner_id=? AND customer_id=?',(uid,cid));_audit(c,uid,'customer',cid,'deleted',{'name':row['name']});c.execute('DELETE FROM customers WHERE id=? AND owner_id=?',(cid,uid));return self.send_json(200,{'ok':True})
