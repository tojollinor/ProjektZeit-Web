...
def job(app,uid,provider):
    ...
    r=dict(r);r['result']=json.loads(r.pop('result_json'));return r

def start_refresh(app,uid,provider,days=7,scheduled=False):
    ...
    with app.db() as c:
        ...
        settings=c.execute('SELECT * FROM company_sync_settings WHERE provider=?',(provider,)).fetchone()
        ...
        if settings['lease_until']>clock:
            ...
            return {'provider':provider,'state':'waiting','error':'Unternehmensweiter Abgleich läuft bereits.','result':None}
    ...
    def monitor():
        ...
        try:
            ...
            process=subprocess.Popen([sys.executable,str(Path(__file__).resolve()),str(uid),provider,key],env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
            ...
            with app.db() as c:
                ...
                c.execute('UPDATE company_sync_settings SET lease_until=0,next_at=CASE WHEN next_at<? THEN ? ELSE next_at END WHERE provider=?',(int(time.time())+(300 if not ok else 0),int(time.time())+(300 if not ok else 0),provider))
...
def worker(uid,provider,key):
    ...
    try:
        if provider=='zammad':result=zammad_cache_runtime._full_refresh(app,uid,incremental=True)
        else:
            config=feature_runtime.integration_config(app,uid,provider)
            if provider=='teamviewer':result=provider_archive.sync_teamviewer(app.db,uid,config,full=False)
            else:result=provider_archive.sync_starface(app.db,uid,config,full=False)
            with app.db() as c:provider_cache_runtime._set_state(c,uid,provider,'valid','Verbindung erfolgreich geprüft.')
        with app.db() as c:c.execute("UPDATE company_sync_jobs SET state='success',finished_at=?,result_json=? WHERE owner_id=? AND provider=? AND id=?",(st.iso(st.now()),json.dumps(result),uid,provider,key))
    except Exception:
