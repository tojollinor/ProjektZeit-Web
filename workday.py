"""Transactional workday tracking and non-overlapping entry corrections."""
...
def reconcile(c, uid):
    """Persist the complement of productive entries inside each working period."""
    ...
    for session in sessions:
        start = stamp(session['started_at'])
        end = stamp(session['ended_at']) if session['ended_at'] else None
        gaps, cursor = [], start
        for row in productive:
            a, b = stamp(row['started_at']), stamp(row['ended_at']) if row['ended_at'] else None
            ...
            a = max(a, start)
            if a > cursor:
                gaps.append((iso(cursor), iso(a)))
            if b is None:
                cursor = None
                break
            cursor = max(cursor, min(b, end) if end else b)
        if cursor is not None and (end is None or cursor < end):
            gaps.append((iso(cursor), iso(end) if end else None))
        ...
        for a, b in gaps:
            ...
            if match:
                c.execute('UPDATE entries SET ended_at=? WHERE id=?', (b, match['id']))
                keep.add(match['id'])
            else:
                c.execute('INSERT INTO entries(owner_id,project_id,category_id,started_at,ended_at,is_idle,work_session_id) VALUES(?,?,?,?,?,1,?)',
                          (uid, pid, cid, a, b, session['id']))
        for row in old:
...
def transition(c, uid, action, body, now):
    ...
    current = c.execute('SELECT * FROM work_sessions WHERE owner_id=? AND ended_at IS NULL', (uid,)).fetchone()
    t = iso(now)
    if action == 'begin':
        if current:
            raise ValueError('Der Arbeitstag läuft bereits.')
        overlap(c, uid, now, None)
        c.execute('INSERT INTO work_sessions(owner_id,started_at) VALUES(?,?)', (uid, t))
    else:
        if not current:
            raise ValueError('Bitte zuerst Arbeitsbeginn drücken.')
        if action == 'switch':
            pid, cid = body.get('project_id'), body.get('category_id')
            if not c.execute('SELECT 1 FROM projects WHERE id=? AND owner_id=? AND active=1 AND is_system=0', (pid, uid)).fetchone():
                raise ValueError('Bitte ein aktives Projekt auswählen.')
            if not c.execute('SELECT 1 FROM categories WHERE id=? AND owner_id=?', (cid, uid)).fetchone():
                raise ValueError('Bitte eine Zeitkategorie auswählen.')
            running = c.execute('SELECT * FROM entries WHERE owner_id=? AND is_idle=0 AND ended_at IS NULL', (uid,)).fetchone()
            if running and running['project_id'] == pid and running['category_id'] == cid:
                return
        c.execute('UPDATE entries SET ended_at=? WHERE owner_id=? AND is_idle=0 AND ended_at IS NULL', (t, uid))
        if action == 'end':
            c.execute('UPDATE work_sessions SET ended_at=? WHERE id=?', (t, current['id']))
        elif action == 'switch':
            overlap(c, uid, now, None)
            c.execute('INSERT INTO entries(owner_id,project_id,category_id,started_at,note,work_session_id) VALUES(?,?,?,?,?,?)',
                      (uid, pid, cid, t, str(body.get('note', ''))[:2000], current['id']))
    reconcile(c, uid)
...
def edit(c, uid, body, now):
    ...
    if not row:
        raise ValueError('Stempelung nicht gefunden.')
    if body.get('original_start') != row['started_at'] or body.get('original_end') != row['ended_at'] or body.get('original_note') != row['note']:
        raise ValueError('Die Stempelung wurde inzwischen geändert. Bitte neu öffnen.')
    note = str(body.get('note', ''))[:2000]
    if not row['ended_at'] or (row['is_idle'] and not body.get('project_id')):
        c.execute('UPDATE entries SET note=? WHERE id=?', (note, row['id']))
        return
    a, b = stamp(body['started_at']), stamp(body['ended_at'])
    if b <= a or b > now:
        raise ValueError('Das Ende muss nach dem Start und darf nicht in der Zukunft liegen.')
    pid, cid = body.get('project_id'), body.get('category_id')
    if not c.execute('SELECT 1 FROM projects WHERE id=? AND owner_id=? AND is_system=0', (pid, uid)).fetchone() or not c.execute('SELECT 1 FROM categories WHERE id=? AND owner_id=?', (cid, uid)).fetchone():
        raise ValueError('Projekt oder Kategorie ist ungültig.')
    if row['work_session_id']:
        work = c.execute('SELECT * FROM work_sessions WHERE id=?', (row['work_session_id'],)).fetchone()
        if a < stamp(work['started_at']) or (work['ended_at'] and b > stamp(work['ended_at'])):
            raise ValueError('Die Stempelung muss innerhalb ihrer Arbeitszeit bleiben.')
    overlap(c, uid, a, b, row['id'])
    c.execute('UPDATE entries SET project_id=?,category_id=?,started_at=?,ended_at=?,note=?,is_idle=0 WHERE id=?', (pid, cid, iso(a), iso(b), note, row['id']))
    reconcile(c, uid)
